...
<template>
  <div class="h-screen flex flex-col bg-bg-page">
    <!-- Top bar -->
    <header class="h-14 flex shrink-0 items-center border-b border-border-light bg-bg-surface px-6">
    ...
    <div class="flex flex-1 overflow-hidden">
      <!-- Left nav -->
      <aside class="hidden w-60 shrink-0 overflow-y-auto border-r border-border-light bg-bg-surface md:block lg:w-60 md:w-16">
        <slot name="nav" />
      </aside>

      <!-- Main work area -->
      <main class="min-w-0 flex-1 overflow-y-auto lg:min-w-720px">
        <slot />
      </main>
