...
1. 使用 `pnpm test:coverage` 执行各包覆盖率门禁；门禁只允许随测试增加而提高，不得为了通过 CI 下调。
2. API 集成测试必须使用名称以 `_test` 结尾的独立 PostgreSQL 数据库。
3. 可通过 `TEST_DATABASE_URL` 指定测试库；测试代码不得复用 `DATABASE_URL` 指向的开发库或生产库。
4. 集成测试负责迁移测试库并在用例之间清理数据，禁止依赖人工准备的业务数据。
