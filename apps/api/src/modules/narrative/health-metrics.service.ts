...
interface ChapterRiskInput {
  id: string
  chapterNumber: number
}

interface ForeshadowingRiskInput {
  title: string
  status: string
  expectedPayoffChapterId?: string | null
}

interface QualityReportRiskInput {
...
interface StyleFingerprintRiskInput {
...
export function computeForeshadowingRisk(
  projectId: string,
  allForeshadowing: ForeshadowingRiskInput[],
  completedChapters: number,
  allChapters: ChapterRiskInput[],
): HealthRisk[] {
...
export function computeCharacterOOC(
  projectId: string,
  reports: QualityReportRiskInput[],
  allChapters: ChapterRiskInput[],
): HealthRisk[] {
  const risks: HealthRisk[] = []
...
export function computePacingRisk(
  ...
  if (weakRhythmReports.length > 0) {
    risks.push({
      id: 'low-rhythm-score',
      severity: weakRhythmReports.some(r => (r.rhythmScore ?? 0) < 45) ? 'high' : 'medium',
      type: 'pacing',
      title: '章节节奏评分偏低',
...
export function computeStyleDrift(
  projectId: string,
  fingerprints: StyleFingerprintRiskInput[],
): HealthRisk[] {
  const risks: HealthRisk[] = []
