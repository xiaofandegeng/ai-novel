...
export async function runChapterPostprocess(input: {
  ...
  try {
    ...
    const charChangesStr = Array.isArray(parsed.characterStateChanges) && parsed.characterStateChanges.length
    ...
    const fields = {
      ...
      newFacts: factsStr,
      characterStateChanges: charChangesStr,
    ...
    if (Array.isArray(parsed.characterStateChanges) && parsed.characterStateChanges.length) {
    ...
    const conflictUpdatesToReturn: ChapterPostprocessResult['conflictUpdates'] = []
